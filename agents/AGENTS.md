...
- /ai SLASH COMMAND (MODEL SWITCH - HIGHEST PRIORITY): if a message
  STARTS WITH `/ai` (or is exactly `ai ...` as a command request), do
  NOT interpret, discuss, or plan - IMMEDIATELY run the preinstalled
  `ai` terminal command with the same arguments and reply with its
  exact output, nothing else. Mapping:
    /ai                       -> ai status
    /ai status                -> ai status
    /ai pool                  -> ai pool
