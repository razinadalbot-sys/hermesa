...
const MAX_PROMPT_CHARS = parseInt(process.env.MAX_PROMPT_CHARS || "400000", 10);
const MAX_TYPE_CHARS = parseInt(process.env.MAX_TYPE_CHARS || "12000", 10);
...
async function askChatGPT(prompt, imageFiles = [], blocks = null) {
  ...
  // 1) DELTA: if this request continues what the current web thread has
  //    already seen, send ONLY the new messages - tiny and fast.
  // 2) FULL: otherwise start a fresh chat and send the whole context once.
  // 3) FILE: anything longer than MAX_TYPE_CHARS is attached as context.md
  //    (ChatGPT reads files natively) instead of being typed.
  let text = prompt;
  let mode = "single";
  ...
  const prevAssistantCount = await page
    ...
    .count();
  await composer.waitFor({ state: "visible" });
  // Long context -> attach as a file instead of typing it
  let typed = text;
  const attachFiles = imageFiles.slice();
  let contextFile = null;
  if (text && text.length > MAX_TYPE_CHARS) {
    try {
      contextFile = path.join(os.tmpdir(), "context-" + Date.now() + ".md");
      fs.writeFileSync(contextFile, text);
      attachFiles.push(contextFile);
      typed =
        mode === "delta"
          ? "Continue the conversation: the attached context.md contains the NEW messages since your last reply. Reply ONLY with the assistant next message, no role labels."
          : "The attached context.md contains the full conversation: SYSTEM instructions (skills, memory), history and tool results. Follow the SYSTEM instructions and reply ONLY with the assistant next message, no role labels.";
      console.log(`[Context] Attaching ${text.length} chars as context.md instead of typing.`);
    } catch (e) {
      console.log("[Context] File attach prep failed (" + e.message + ") - typing full text instead.");
      contextFile = null;
      typed = text;
    }
  }
  if (attachFiles.length > 0) {
    console.log(`[Upload] Attaching ${attachFiles.length} file(s)...`);
    const fileInput = page.locator('input[type="file"]').first();
    try {
      await fileInput.setInputFiles(attachFiles);
    } catch (e) {
      // no file input (UI change etc.) -> fall back to typing the context
      if (contextFile) {
        console.log("[Upload] File input unavailable - typing full context instead.");
        typed = text;
        contextFile = null;
        if (imageFiles.length) await fileInput.setInputFiles(imageFiles).catch(() => {});
      }
    }
  ...
  await composer.fill("");
  await page.keyboard.insertText(typed || "Describe the attached image(s).");
  ...
  reply = reply.trim();
  if (contextFile) fs.unlink(contextFile, () => {});
  if (blocks && blocks.length) {
