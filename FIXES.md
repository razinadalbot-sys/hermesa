...
### ChatGPT login credentials moved to the config website (no more hardcoded secrets)

- **Problem**: the bridge shipped with a hardcoded login email + Yahoo app
  password inside `bridge/chatgpt_server.js` and `chatgpt-api.yml` - anyone
  who saw the repo saw the credentials.
- **Fix**:
  - Panel (config website) Web-API section now has two new fields:
    **ChatGPT login email** and **Yahoo app password** (masked input).
    Saved to the vault as `CHATGPT_EMAIL` / `CHATGPT_APP_PASSWORD`
    (both treated as SECRET - masked in run logs).
  - `scripts/firebase_vault.py` loads them from the vault (repo secrets
    `CHATGPT_EMAIL` / `CHATGPT_APP_PASSWORD` work as fallback).
  - ALL hardcoded defaults removed from `bridge/chatgpt_server.js`,
    `chatgpt-api.yml` and `hermes.yml`.
  - If the credentials are missing, the local bridge step skips cleanly
    with a warning (`::warning::ChatGPT bridge NOT started...`) - the main
    run is never affected; pool models keep working.
